...
void veerLeft() {motor1.run(BACKWARD); motor3.run(BACKWARD); delay(400); motor1.run(FORWARD); motor3.run(FORWARD);} // veering left? set left motor backwards for 400ms
void veerRight() {motor2.run(BACKWARD); motor4.run(BACKWARD); delay(400); motor1.run(FORWARD); motor4.run(FORWARD);} // veering right? set left motor backwards for 400ms
...
void checkBackward() { if (motorSet=="BACKWARD") {motor1.run(BACKWARD); motor2.run(BACKWARD); motor3.run(FORWARD); motor4.run(FORWARD); } } // make sure motors are going backward
